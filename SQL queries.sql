...
LEFT JOIN Brand B ON RI.barcode = B.barcode AND RI.BrandID = B.BrandID
...
WHERE ScanRank <= 5;
# Compare the ranking difference between Question 1 and Question 2


-- 3. When considering average spend from receipts with 'rewardsReceiptStatus’ of ‘Accepted’ or ‘Rejected’, which is greater?
SELECT rewardsReceiptStatus, AVG(totalSpent) AS AVG_Spend FROM Receipts
WHERE rewardsReceiptStatus IN ('Accepted', 'Rejected') ## Actually Receipts Data Schema Use "FINISHED" Instead 
GROUP BY 1;
# Compare the average spend based on its Rewards Receipt Status to observe which is greater
...
WHERE rewardsReceiptStatus IN ('Accepted', 'Rejected') ## Actually Receipts Data Schema Use "FINISHED" Instead 
GROUP BY 1;
# Compare the total number of items purchased based on its Rewards Receipt Status to observe which is greater
